      ...
      <div v-for="room in rooms" :key="room.id" class="card-hover p-5 cursor-pointer" @click="joinRoom(room)">
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-bn-muted mt-0.5">{{ room.description || 'Anonymous chat room' }}</p>
            <div class="flex items-center gap-3 mt-2">
              <span class="text-xs text-bn-green font-mono">&#9679; {{ room.online_count || 0 }} online</span>
              <span class="text-xs text-bn-muted font-mono">{{ room.member_count || 0 }} total</span>
            </div>
          </div>
        </div>
      </div>
